...
inputFile <- "data/greedy_data-3.txt"
...
GreedyMotifSearch <- function(dnaStrings, k, t){
  ...
  updateBestMotifsFromAnIteration <- function(initialMotif){
    ...
    tmp <- sapply(2:t, function(i){
      profile <- motifsToProfile(motifs=curMotifs[1:(i-1)])
...
motifs <- GreedyMotifSearch(dnaStrings, k, t)
cat(motifs)